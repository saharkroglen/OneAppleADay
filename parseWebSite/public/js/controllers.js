  ...
  if (getQueryParams('cid')) {
    ...
    query.find({
      ...
      success: function(results) {		
		...
		getImageDataURL($scope.itemBackgroundImage, onSuccess, onError,results[0].attributes.itemColor) ; 
...
$scope.getapp = function () {
   var destinationUrl = 'http://google.com';
	//$location.path(destinationUrl);
   $window.location.href = destinationUrl;
   //$scope.post = {url: 'http://google.com', title: ''};
  };    */
 ...
 *
 */
function getImageDataURL(url, success, error, itemColor) {
    ...
    img.onload = function(){
        ...
        try{           
            ...
            success({image:img, data:data, backgroundColor:itemColor});
        }catch(e){
            error(e);
        }
    }
...
var onSuccess = function(args){  
	...
	{
		$( ".container" ).css("background-color",args.backgroundColor);
		rgb = hexToRgb(args.backgroundColor);
		//return;
	}
	else
	{
		rgb = getAverageRGB(document.getElementById('itemImage'));
    ...
    if(o > 125) {
		...
		$('.icon-strip').css('color', 'black');
    }else{ 
        $('.page-header').css('color', 'white');
		$('.icon-strip').css('color', 'white');
    }
